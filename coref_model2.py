...
class CorefModel(object):
  def __init__(self, config):
    ...
    if config["use_augmented"]:
      self.pos_tag_dict = util.load_tags["pos_tag_path"]
      self.ner_tag_dict = util.load_tags["ner_tag_path"]
    else:
      self.pos_tag_dict = None
      self.ner_tag_dict = None
    ...
    input_props.append((tf.float32, [None, None, self.context_embeddings.size])) # categories with glove embeddings
  ...
  def tensorize_example(self, example, is_training):
    ...
    cat_glove_emb = np.zeros([len(sentences), max_sentence_length, self.context_embeddings.size])
    ...
    if self.config["use_augmented"]:
      for i, category in enumerate(categories):
        for j, cat in enumerate(category):
          tmp_arr = np.zeros([self.glove_embedding_size])

          if len(cat) > 0:
            for word2 in cat:
              my_glove = self.glove_embedding_dict[word2]
              tmp_arr += my_glove

            tmp_arr /= float(len(cat))
          cat_glove_emb[i, j, :] = util.func_normalize(tmp_arr)
    ...
    example_tensors = (tokens, context_word_emb, head_word_emb, lm_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, pos_tag_emb, ner_tag_emb, cat_glove_emb)
  ...
  def truncate_example(self, tokens, context_word_emb, head_word_emb, lm_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, pos_tag_emb, ner_tag_emb, cat_glove_emb):
    ...
    return tokens, context_word_emb, head_word_emb, lm_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, pos_tag_emb, ner_tag_emb, cat_glove_emb
  ...
  def get_predictions_and_loss(self, tokens, context_word_emb, head_word_emb, lm_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, pos_tags, ner_tags, cat_glove):
    ...
    if self.config["use_augmented"]:
      context_emb_list.append(pos_tags)
      context_emb_list.append(ner_tags)
      context_emb_list.append(cat_glove)
