...
class CorefModel(object):
      ...
      def _enqueue_loop():
        while True:
          ...
          for i, example in enumerate(train_examples):
            if self.config["l_adapted"]:
              global_step = session.run(self.global_step)
              p = float(global_step) / float(self.config["stopping_criteria"])              
              # p = float(i) / len(train_examples)
              self.l = 2.0 / (1.0 + np.exp(-10.0 * p)) - 1

            tensorized_example = self.tensorize_example(example, is_training=True, l=self.l)
  ...
  # ==================================================================
  def tensorize_example(self, example, is_training, oov_counts=None, l=1.0):
    ...
    if self.config["use_ner_g"] or self.config["use_ner_phi"]:
      ...
      ner_ids = np.array([self.ner_tag_dict[s[2:].replace('*','')] if len(s) > 1 else self.ner_tag_dict[s] for s in util.flatten(ner_tags)])
    ...
    ner_tag_emb = np.zeros([len(sentences), max_sentence_length, len(self.ner_tag_dict)])
    categories_emb = np.zeros([len(sentences), max_sentence_length, len(self.categories_dict)])
    ...
    # domain_labels = np.zeros([len(sentences), len(self.genres)])
    
    domain_labels = np.zeros(len(self.genres))
    if self.config["use_dann"]:
      domain_labels[genre] = 1

      # for i in xrange(len(sentences)):
      #   domain_labels[i, genre] = 1 # should I do this? or one genre per document

    gold_starts, gold_ends = self.tensorize_mentions(gold_mentions)

    if is_training and len(sentences) > self.config["max_training_sentences"]:
      return self.truncate_example(word_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, pos_tag_emb, ner_tag_emb, categories_emb, ner_ids, cat_glove_emb, domain_labels, l)
    else:
      return word_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, pos_tag_emb, ner_tag_emb, categories_emb, ner_ids, cat_glove_emb, domain_labels, l


  def truncate_example(self, word_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, pos_tag_emb, ner_tag_emb, categories_emb, ner_ids, cat_glove_emb, domain_labels, l):
    ...
    ner_tag_emb = ner_tag_emb[sentence_offset:sentence_offset + max_training_sentences,:,:]
  ...
  # modify this for domain adaptation purpose
  # genre in the parameter can be the label
  def get_predictions_and_loss(self, word_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, pos_tags, ner_tags, categories, ner_ids, cat_glove, domain_labels, l):
    ...
    antecedent_scores = self.get_antecedent_scores(mention_emb, mention_scores, antecedents, antecedents_len, mention_starts, mention_ends, mention_speaker_ids, genre_emb, mention_ner_ids) # [num_mentions, max_ant + 1]
    ...
    d_logits = util.dann(candidate_mention_emb, self.config["ffnn_size"], 
                              len(self.genres), self.dropout, l=l)# l=tf.constant(self.l))
    ...
    # self.domain_accuracy = domain_accuracy

    values = [domain_accuracy, domain_loss_reduce_mean, pairwise_loss_reduced, N, neg_ll]
  ...
  def get_antecedent_scores(self, mention_emb, mention_scores, antecedents, antecedents_len, mention_starts, mention_ends, mention_speaker_ids, genre_emb, mention_ner_ids):
    ...
    return antecedent_scores #, [antecedent_speaker_ids, antecedents, speaker_pair_emb, same_speaker, mention_speaker_ids, antecedent_ner_ids, mention_ner_ids, same_ner, ner_pair_emb]  # [num_mentions, max_ant + 1]
  ...
  def get_predicted_clusters(self, mention_starts, mention_ends, predicted_antecedents):
    ...
    for i, predicted_index in enumerate(predicted_antecedents):
      ...
      if predicted_index < 0 or i <= predicted_index:
        continue
      assert i > predicted_index # why is i == predicted_index == 0?
  ...
  def evaluate(self, session, official_stdout=False):
    ...
    for example_num, (tensorized_example, example) in enumerate(self.eval_data):
      _, _, _, _, example_genre, _, gold_starts, gold_ends, _, _, _, _, _, _, raw_gold_domains, _ = tensorized_example
      feed_dict = {i:t for i,t in zip(self.input_tensors, tensorized_example)}
            
      predictions, pred_domains = session.run([self.predictions, self.domain_predictions], feed_dict=feed_dict)
      [candidate_starts, candidate_ends, mention_scores, mention_starts, mention_ends, antecedents, antecedent_scores, antecedent_labels, genre] = predictions
      ...
      gold_domains = np.tile(np.argmax(raw_gold_domains), pred_domains.shape[0])
      total_correct += np.sum(np.equal(gold_domains, pred_domains))
      mention_count += pred_domains.shape[0]
    ...
    # domain_accuracy = np.sum(np.equal(gold_domains, pred_domains)) / float(pred_domains.shape[0])
    domain_accuracy = total_correct / float(mention_count)
    print "Domain accuracy: {:.2f}%".format(domain_accuracy * 100)
